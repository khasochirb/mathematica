...
import { getIbAiSlSpine } from "@/lib/genmath-data/ib-ai-sl";
import CoursePersonalization from "@/components/course/CoursePersonalization";

// The IB Mathematics AI SL course hub — Applications & Interpretation,
// taught by the official syllabus: five topics, each lesson one subtopic
// code (AI SL 1.1, AI SL 1.2, …), formulas flagged booklet/memorize,
// solutions to markscheme standard. The sibling of /math/ib-sl (Analysis &
// Approaches); AI is calculator-always and leans on modelling. Lives under
// /math for the course machinery; the IB hub at /practice/ib is the front
// door.
export default function IbAiSlCoursePage() {
  const spine = getIbAiSlSpine();
    ...
    <div className="min-h-screen pt-20" style={{ background: "var(--bg)" }}>
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 pb-16">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div className="eyebrow">IB Mathematics · Applications &amp; Interpretation</div>
        </div>
        ...
        <p className="mt-4 mb-4" style={{ color: "var(--fg-1)", fontSize: 17, maxWidth: "56ch" }}>
          The Applications &amp; Interpretation SL syllabus, taught topic by
          topic and code by code — every lesson is one official subtopic
          (AI SL 1.1, AI SL 1.2, …), every formula flagged as in-the-booklet
          or memorize, and every worked example solved the way an IB examiner
        ...
        <ol className="space-y-3">
          {spine.map((u) => {
            return u.live ? (
              <li key={u.slug}>
                <TopicLink courseKey="ib-ai-sl" topicSlug={u.slug}                   href={`/math/ib-ai-sl/${u.slug}`}
