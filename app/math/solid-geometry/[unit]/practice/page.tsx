...
import { getSolidGeoUnit } from "@/lib/genmath-data/solid-geometry";
...
function SolidGeoPracticePageInner() {
    ...
    <div className="min-h-screen pt-20" style={{ background: "var(--bg)" }}>
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 pb-16">
        ...
        <div className="flex items-center gap-3 mb-6">
          <Link
            href={`/math/solid-geometry/${unitSlug}`}
            className="p-2 rounded-md transition-colors"
            style={{ background: "var(--bg-2)", border: "1px solid var(--line)", color: "var(--fg-2)" }}
          >
            <ArrowLeft className="w-4 h-4" />
          </Link>
          <div className="eyebrow">Solid Geometry · Unit {unit.unit} · {unit.title}</div>
        </div>
