...
import { getGrade4Spine, getGrade4TopicLocalized } from "@/lib/genmath-data/grade-4";
import { useLang } from "@/lib/lang-context";

// Grade 4 — the first PRIMARY-band course. The grade goes live topic by
...
export default function Grade4TopicsPage() {
  const spine = getGrade4Spine();
  const { lang } = useLang();
  const mn = lang === "mn";
    ...
    <div className="min-h-screen pt-20" style={{ background: "var(--bg)" }}>
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 pb-16">
        ...
        <h1 className="serif" style={{ fontWeight: 400, fontSize: "clamp(32px, 5vw, 56px)", letterSpacing: "-0.04em", lineHeight: 1, color: "var(--fg)" }}>
          {mn ? "4-р ангийн сэдвүүд" : "Grade 4 Topics"}
        </h1>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {spine.map((entry) => {
            ...
            return (
              <Link key={entry.slug} href={`/math/4/${entry.slug}`} className="card-edit p-5 flex flex-col gap-1.5 transition-colors group" style={{ textDecoration: "none" }}>
        ...
        <p className="text-[13px] mt-8" style={{ color: "var(--fg-3)" }}>
          {complete
            ? mn
              ? `Бүх ${spine.length} сэдэв нээлттэй — 4-р ангийн бүтэн хөтөлбөр.`
              : `All ${spine.length} topics are open — the complete Grade 4 year.`
            : mn
