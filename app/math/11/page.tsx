...
import { getGrade11Spine, getGrade11Topic } from "@/lib/genmath-data/grade-11";
import { useLang } from "@/lib/lang-context";
import { useAuth } from "@/lib/auth-context";
import { loadPlacement, type StoredPlacement } from "@/lib/placement-result";

// Grade 11 hub — the full 7-topic course, with the adaptive placement test up
// top and "important for you" badges once a result exists.
export default function Grade11TopicsPage() {
  const { lang } = useLang();
  const mn = lang === "mn";
  const spine = getGrade11Spine();
  const { user } = useAuth();
  ...
  useEffect(() => {
    setPlacement(loadPlacement(user?.id, "grade11"));
  }, [user?.id]);
    ...
    <div className="min-h-screen pt-20" style={{ background: "var(--bg)" }}>
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 pb-16">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div className="eyebrow">{mn ? "Хичээлүүд · 11-р анги" : "Courses · Grade 11"}</div>
        </div>

        <h1 className="serif" style={{ fontWeight: 400, fontSize: "clamp(32px, 5vw, 56px)", letterSpacing: "-0.04em", lineHeight: 1, color: "var(--fg)" }}>
          {mn ? "11-р ангийн сэдвүүд" : "Grade 11 Topics"}
        </h1>
        <p className="mt-3 mb-8" style={{ color: "var(--fg-1)", fontSize: 16, maxWidth: "56ch" }}>
          The functions year — transformations, polynomials, logarithms, sequences,
          the unit circle, complex numbers, and real statistics. Each topic is
          taught from the ground up with interactive, self-checking lessons.
        </p>

        {/* Placement CTA / summary */}
        {placement ? (
          <Link href="/math/11/placement" className="card-edit p-4 mb-8 flex items-center gap-4" style={{ textDecoration: "none", borderColor: "var(--accent-line)", background: "var(--accent-wash)" }}>
        ...
        <ol className="space-y-3">
          {spine.map((t, i) => {
            const topic = t.live ? getGrade11Topic(t.slug) : null;
            const lessonCount = topic?.lessons.length ?? 0;
            const important = prioritySet.has(t.slug);
            return t.live ? (
              <li key={t.slug}>
                <TopicLink courseKey="11" topicSlug={t.slug}                   href={`/math/11/${t.slug}`}
