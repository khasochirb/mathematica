...
import { getGrade11TopicLocalized, getGrade11Lesson } from "@/lib/genmath-data/grade-11";
...
function GenMathLessonPageInner() {
  ...
  const lesson = getGrade11Lesson(topicSlug, lessonSlug);
  ...
  if (lesson.interactive) {
    return <LessonPlayer lesson={lesson} topicSlug={topicSlug} topicTitle={topic.title} baseHref={`/math/11/${topicSlug}`} crumb={`Courses · Grade 11 · ${topic.title}`} />;
  }
    ...
    <div className="min-h-screen pt-20" style={{ background: "var(--bg)" }}>
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 pb-16">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div className="eyebrow">Courses · Grade 11 · {topic.title}</div>
        </div>
