    ...
    return (
      <div className="min-h-screen pt-20 flex items-center justify-center" style={{ background: "var(--bg)" }}>
        <div className="text-center">
          ...
          <Link href="/math/7" className="btn btn-line mt-5 inline-flex items-center gap-1.5">
            <ArrowLeft className="h-3.5 w-3.5" /> Back to Grade 7
          </Link>
        </div>
      </div>
    );
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 pb-16">
        ...
        <ol className="space-y-3 mb-10">
          {topic.lessons.map((lesson, i) => (
            <li key={lesson.slug}>
              <Link
                href={`/math/7/${topicSlug}/${lesson.slug}`}
        ...
        <div className="flex gap-3 flex-wrap">
          <Link href={`/math/7/${topicSlug}/practice`} className="btn btn-primary">
            {mn ? "Дасгал" : "Practice"}
          </Link>
          <Link href={`/math/7/${topicSlug}/test`} className="btn btn-line">
            {mn ? "Өөрийгөө шалга" : "Test yourself"}
          </Link>
          <Link href={`/math/problem-bank/7/${topicSlug}`} className="btn btn-line">
