...
// Grade 7 hub — built one topic at a time. The full roadmap is visible from
// day one: live topics link in, the rest show the road ahead.
export default function Grade7TopicsPage() {
  const spine = getGrade7Spine();
  const { user } = useAuth();
  const { lang } = useLang();
  const mn = lang === "mn";
  ...
  useEffect(() => {
    setPlacement(loadPlacement(user?.id, "grade7"));
  }, [user?.id]);
    ...
    <div className="min-h-screen pt-20" style={{ background: "var(--bg)" }}>
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 pb-16">
        ...
        <h1 className="serif" style={{ fontWeight: 400, fontSize: "clamp(32px, 5vw, 56px)", letterSpacing: "-0.04em", lineHeight: 1, color: "var(--fg)" }}>
          {mn ? "7-р ангийн сэдвүүд" : "Grade 7 Topics"}
        </h1>
        <p className="mt-3 mb-8" style={{ color: "var(--fg-1)", fontSize: 16, maxWidth: "56ch" }}>
          {mn
            ? "Пропорциональ сэтгэлгээний жил — харьцаа ба хурдац хаа сайгүй, сөрөг тоонууд дөрвөн үйлдэлд бүрэн орж, хоёр алхамт тэгшитгэл, хувийн хэрэглээ, масштаб ба тойрог, магадлал, түүвэрлэлт. Сэдэв бүрийг интерактив, өөрийгөө шалгадаг хичээлүүдээр сууриас нь зааж өгнө."
            : "The year of proportional thinking — ratios and rates everywhere, negative numbers in all four operations, two-step equations, percent in the real world, scale and circles, probability, and sampling. Each topic is taught from the ground up with interactive, self-checking lessons."}
        </p>

        {/* Placement CTA / summary */}
        {placement ? (
          <Link href="/math/7/placement" className="card-edit p-4 mb-8 flex items-center gap-4" style={{ textDecoration: "none", borderColor: "var(--accent-line)", background: "var(--accent-wash)" }}>
        ...
        <ol className="space-y-3">
          {spine.map((t, i) => {
            ...
            return t.live ? (
              <li key={t.slug}>
                <TopicLink courseKey="7" topicSlug={t.slug}                   href={`/math/7/${t.slug}`}
