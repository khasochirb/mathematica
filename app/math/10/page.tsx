...
import { getGrade10Spine, getGrade10Topic } from "@/lib/genmath-data/grade-10";
import { useLang } from "@/lib/lang-context";
import { useAuth } from "@/lib/auth-context";
import { loadPlacement, type StoredPlacement } from "@/lib/placement-result";

// Grade 10 hub — the full 7-topic course, with the adaptive placement test up
// top and "important for you" badges once a result exists.
export default function Grade10TopicsPage() {
  const { lang } = useLang();
  const mn = lang === "mn";
  const spine = getGrade10Spine();
  const { user } = useAuth();
  ...
  useEffect(() => {
    setPlacement(loadPlacement(user?.id, "grade10"));
  }, [user?.id]);
    ...
    <div className="min-h-screen pt-20" style={{ background: "var(--bg)" }}>
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 pb-16">
        ...
        <h1 className="serif" style={{ fontWeight: 400, fontSize: "clamp(32px, 5vw, 56px)", letterSpacing: "-0.04em", lineHeight: 1, color: "var(--fg)" }}>
          {mn ? "10-р ангийн сэдвүүд" : "Grade 10 Topics"}
        </h1>
        <p className="mt-3 mb-8" style={{ color: "var(--fg-1)", fontSize: 16, maxWidth: "56ch" }}>
          Algebra grows teeth — polynomials, quadratics in every form, rational and
          radical expressions, exponential growth, and probability. Each topic is
          taught from the ground up with interactive, self-checking lessons.
        </p>

        {/* Placement CTA / summary */}
        {placement ? (
          <Link href="/math/10/placement" className="card-edit p-4 mb-8 flex items-center gap-4" style={{ textDecoration: "none", borderColor: "var(--accent-line)", background: "var(--accent-wash)" }}>
        ...
        <ol className="space-y-3">
          {spine.map((t, i) => {
            const topic = t.live ? getGrade10Topic(t.slug) : null;
            const lessonCount = topic?.lessons.length ?? 0;
            const important = prioritySet.has(t.slug);
            return t.live ? (
              <li key={t.slug}>
                <TopicLink courseKey="10" topicSlug={t.slug}                   href={`/math/10/${t.slug}`}
