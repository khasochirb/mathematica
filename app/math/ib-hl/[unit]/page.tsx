...
export default function IbHlUnitPage() {
  ...
  const spineEntry = getIbHlSpine().find((u) => u.slug === unitSlug);
    ...
    <div className="min-h-screen pt-20" style={{ background: "var(--bg)" }}>
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 pb-16">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div className="eyebrow">IB Math HL (AA) · Topic {unit.unit}</div>
        </div>
        ...
        <ol className="space-y-3">
          {unit.lessons.map((lesson, i) => (
            <li key={lesson.slug}>
              <Link
                href={`/math/ib-hl/${unitSlug}/${lesson.slug}`}
          ...
          <>
            ...
            <div className="flex flex-wrap gap-3">
              {unit.practice.length > 0 && (
                <Link href={`/math/ib-hl/${unitSlug}/practice`} className="btn btn-primary">
                  Practice
                </Link>
              )}
              {unit.testYourself.length > 0 && (
                <Link href={`/math/ib-hl/${unitSlug}/test`} className="btn btn-line">
                  Test yourself
                </Link>
              )}
            </div>
