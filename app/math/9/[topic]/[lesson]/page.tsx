...
function GenMathLessonPageInner() {
  ...
  if (lesson.interactive) {
    return <LessonPlayer lesson={lesson} topicSlug={topicSlug} topicTitle={topic.title} baseHref={`/math/9/${topicSlug}`} crumb={`${mn ? "Хичээлүүд · 9-р анги" : "Courses · Grade 9"} · ${topic.title}`} />;
  }
    ...
    <div className="min-h-screen pt-20" style={{ background: "var(--bg)" }}>
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 pb-16">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div className="eyebrow">{mn ? "Хичээлүүд · 9-р анги" : "Courses · Grade 9"} · {topic.title}</div>
        </div>
...
export default function GenMathLessonPage() {
  ...
  const topicSlug = params.topic as string;
  return (
    <ContentGate courseKey="9" topicSlug={topicSlug} backHref={`/math/9/${topicSlug}`} backLabel="Back to topic">
