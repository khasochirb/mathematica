...
import { getGrade9Spine, getGrade9Topic, getGrade9TopicLocalized } from "@/lib/genmath-data/grade-9";
...
export default function Grade9TopicsPage() {
  ...
  const priorityTitles = (placement?.priorityTopics ?? [])
    .map((slug) => getGrade9TopicLocalized(slug, lang)?.title ?? spine.find((t) => t.slug === slug)?.title)
    ...
    <div className="min-h-screen pt-20" style={{ background: "var(--bg)" }}>
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 pb-16">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div className="eyebrow">{mn ? "Хичээлүүд · 9-р анги" : "Courses · Grade 9"}</div>
        </div>
        ...
        <ol className="space-y-3">
          {spine.map((t, i) => {
            const topic = t.live ? getGrade9TopicLocalized(t.slug, lang) : null;
            ...
            return t.live ? (
              <li key={t.slug}>
                <TopicLink courseKey="9" topicSlug={t.slug}                   href={`/math/9/${t.slug}`}
