...
import { getGrade2Spine, getGrade2TopicLocalized } from "@/lib/genmath-data/grade-2";
import { useLang } from "@/lib/lang-context";

// Grade 2 — the lowest year currently open, and the rung below Grade 4. It
...
export default function Grade2TopicsPage() {
  const spine = getGrade2Spine();
  const { lang } = useLang();
  const mn = lang === "mn";
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 pb-16">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {spine.map((entry) => {
            ...
            const topic = getGrade2TopicLocalized(entry.slug, lang);
            const lessonCount = topic?.lessons.length ?? 0;
            return (
              <TopicLink courseKey="3" topicSlug={entry.slug} key={entry.slug} href={`/math/2/${entry.slug}`} className="card-edit p-5 flex flex-col gap-1.5 transition-colors group" style={{ textDecoration: "none" }}>
        ...
        <p className="text-[13px] mt-8" style={{ color: "var(--fg-3)" }}>
          {complete
            ? mn
              ? `Бүх ${spine.length} сэдэв нээлттэй — 2-р ангийн бүтэн хөтөлбөр.`
              : `All ${spine.length} topics are open — the complete Grade 2 year.`
            : mn
