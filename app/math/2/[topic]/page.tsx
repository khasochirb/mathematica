...
import { getGrade2TopicLocalized } from "@/lib/genmath-data/grade-2";
...
export default function GenMathTopicPage() {
  ...
  const topic = getGrade2TopicLocalized(topicSlug, lang);
    ...
    return (
      <div className="min-h-screen pt-20 flex items-center justify-center" style={{ background: "var(--bg)" }}>
        <div className="text-center">
          ...
          <Link href="/math/2" className="btn btn-line mt-5 inline-flex items-center gap-1.5">
            <ArrowLeft className="h-3.5 w-3.5" /> Back to Grade 2
          </Link>
        </div>
      </div>
    );
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 pb-16">
        ...
        <ol className="space-y-3 mb-10">
          {topic.lessons.map((lesson, i) => (
            <li key={lesson.slug}>
              <Link
                href={`/math/2/${topicSlug}/${lesson.slug}`}
        ...
        {/* Practice + Test buttons, and this unit's own problem bank — every
            Grade 2 unit has its own collection (see scripts/pb/grade4.py). */}
        <div className="flex gap-3 flex-wrap">
          <Link href={`/math/2/${topicSlug}/practice`} className="btn btn-primary">
            {mn ? "Дасгал" : "Practice"}
          </Link>
          <Link href={`/math/2/${topicSlug}/test`} className="btn btn-line">
            {mn ? "Өөрийгөө шалга" : "Test yourself"}
          </Link>
          <Link href={`/math/problem-bank/2/${topicSlug}`} className="btn btn-line">
