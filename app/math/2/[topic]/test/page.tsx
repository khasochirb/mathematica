    ...
    <div className="min-h-screen pt-20" style={{ background: "var(--bg)" }}>
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 pb-16">
        ...
        <div className="flex items-center gap-3 mb-6">
          <Link
            href={`/math/2/${topicSlug}`}
            className="p-2 rounded-md transition-colors"
            style={{ background: "var(--bg-2)", border: "1px solid var(--line)", color: "var(--fg-2)" }}
          >
            <ArrowLeft className="w-4 h-4" />
          </Link>
          <div className="eyebrow">{mn ? "Хичээлүүд · 2-р анги" : "Courses · Grade 2"} · {topic.title}</div>
        </div>
        ...
        >
          ...
          <p className="text-[13px]" style={{ color: "var(--fg-2)" }}>
            {mn
              ? "Бодлого бүрийг цаасан дээр бодоод, бодолтыг нээж өөрийгөө шударгаар дүгнээрэй — таны дүгнэлт ахицын статистикт тоологдоно."
...
export default function GenMathTestPage() {
  ...
  const topicSlug = params.topic as string;
  return (
    <ContentGate courseKey="3" topicSlug={topicSlug} backHref={`/math/2/${topicSlug}`} backLabel="Back to topic">
