...
import { getGrade6TopicLocalized } from "@/lib/genmath-data/grade-6";
...
function GenMathLessonPageInner() {
  ...
  const topic = getGrade6TopicLocalized(topicSlug, lang);
  ...
  if (lesson.interactive) {
    return <LessonPlayer lesson={lesson} topicSlug={topicSlug} topicTitle={topic.title} baseHref={`/math/6/${topicSlug}`} crumb={`${mn ? "Хичээлүүд · 6-р анги" : "Courses · Grade 6"} · ${topic.title}`} />;
  }
    ...
    <div className="min-h-screen pt-20" style={{ background: "var(--bg)" }}>
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 pb-16">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div className="eyebrow">{mn ? "Хичээлүүд · 6-р анги" : "Courses · Grade 6"} · {topic.title}</div>
        </div>
