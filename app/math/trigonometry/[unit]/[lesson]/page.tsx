...
import { getTrigUnit, getTrigLesson } from "@/lib/genmath-lessons";
...
function TrigLessonPageInner() {
  ...
  const lesson = getTrigLesson(unitSlug, lessonSlug);
  ...
  return (
    <LessonPlayer
      ...
      baseHref={`/math/trigonometry/${unitSlug}`}
      crumb={`Trigonometry · Unit ${unit.unit} · ${unit.title}`}
    />
  );
}
