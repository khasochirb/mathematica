...
// show the road ahead.
export default function ProbStatCoursePage() {
  const spine = getProbStatSpine();
    ...
    <div className="min-h-screen pt-20" style={{ background: "var(--bg)" }}>
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 pb-16">
        ...
        <p className="mt-4 mb-8" style={{ color: "var(--fg-1)", fontSize: 17, maxWidth: "56ch" }}>
          One continuous course in three acts: first learn to COUNT (combinatorics),
          then use counting to measure CHANCE (probability), then use chance to read
        ...
        <CoursePlacementCta
          ...
          href="/math/prob-stats/placement"
        ...
        <ol className="space-y-3">
          {spine.map((u) => {
            return u.live ? (
              <li key={u.slug}>
                <TopicLink courseKey="prob-stats" topicSlug={u.slug}                   href={`/math/prob-stats/${u.slug}`}
