...
import { getProbStatUnit } from "@/lib/genmath-data/prob-stats";
...
function ProbStatPracticePageInner() {
    ...
    return (
      <div className="min-h-screen pt-20 flex items-center justify-center" style={{ background: "var(--bg)" }}>
        <div className="text-center">
          ...
          <Link href="/math/prob-stats" className="btn btn-line mt-5 inline-flex items-center gap-1.5">
    ...
    <div className="min-h-screen pt-20" style={{ background: "var(--bg)" }}>
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 pb-16">
        ...
        <div className="flex items-center gap-3 mb-6">
          <Link
            href={`/math/prob-stats/${unitSlug}`}
            className="p-2 rounded-md transition-colors"
            style={{ background: "var(--bg-2)", border: "1px solid var(--line)", color: "var(--fg-2)" }}
          >
            <ArrowLeft className="w-4 h-4" />
          </Link>
          <div className="eyebrow">Prob & Stats · Unit {unit.unit} · {unit.title}</div>
        </div>
...
export default function ProbStatPracticePage() {
  const params = useParams();
  const { lang } = useLang();
  const unitSlug = params.unit as string;
  return (
    <ContentGate courseKey="prob-stats" topicSlug={unitSlug} backHref={`/math/prob-stats/${unitSlug}`} backLabel={chrome("Back to unit", lang)}>
      <ProbStatPracticePageInner />
    </ContentGate>
  );
}
