...
import { getGrade3TopicLocalized } from "@/lib/genmath-data/grade-3";
...
function GenMathTestPageInner() {
  ...
  const topic = getGrade3TopicLocalized(topicSlug, lang);
    ...
    return (
      <div className="min-h-screen pt-20 flex items-center justify-center" style={{ background: "var(--bg)" }}>
        <div className="text-center">
          ...
          <Link href="/math/3" className="btn btn-line mt-5 inline-flex items-center gap-1.5">
            <ArrowLeft className="h-3.5 w-3.5" /> Back to Grade 3
          </Link>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen pt-20" style={{ background: "var(--bg)" }}>
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 pb-16">
        ...
        <div className="flex items-center gap-3 mb-6">
          <Link
            href={`/math/3/${topicSlug}`}
            className="p-2 rounded-md transition-colors"
            style={{ background: "var(--bg-2)", border: "1px solid var(--line)", color: "var(--fg-2)" }}
          >
            <ArrowLeft className="w-4 h-4" />
          </Link>
          <div className="eyebrow">{mn ? "Хичээлүүд · 3-р анги" : "Courses · Grade 3"} · {topic.title}</div>
        </div>
...
export default function GenMathTestPage() {
  ...
  const topicSlug = params.topic as string;
  return (
    <ContentGate courseKey="3" topicSlug={topicSlug} backHref={`/math/3/${topicSlug}`} backLabel="Back to topic">
