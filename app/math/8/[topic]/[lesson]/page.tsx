...
import { getGrade8TopicLocalized } from "@/lib/genmath-data/grade-8";
...
function GenMathLessonPageInner() {
  ...
  const topic = getGrade8TopicLocalized(topicSlug, lang);
  ...
  if (lesson.interactive) {
    return <LessonPlayer lesson={lesson} topicSlug={topicSlug} topicTitle={topic.title} baseHref={`/math/8/${topicSlug}`} crumb={`${mn ? "Хичээлүүд · 8-р анги" : "Courses · Grade 8"} · ${topic.title}`} />;
  }
    ...
    <div className="min-h-screen pt-20" style={{ background: "var(--bg)" }}>
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 pb-16">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div className="eyebrow">{mn ? "Хичээлүүд · 8-р анги" : "Courses · Grade 8"} · {topic.title}</div>
        </div>
...
export default function GenMathLessonPage() {
  ...
  const topicSlug = params.topic as string;
  return (
    <ContentGate courseKey="8" topicSlug={topicSlug} backHref={`/math/8/${topicSlug}`} backLabel="Back to topic">
