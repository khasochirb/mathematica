...
export default function Grade8TopicsPage() {
  ...
  useEffect(() => {
    setPlacement(loadPlacement(user?.id, "grade8"));
  }, [user?.id]);

  const prioritySet = new Set(placement?.priorityTopics ?? []);
  const priorityTitles = (placement?.priorityTopics ?? [])
    .map((slug) => getGrade8TopicLocalized(slug, lang)?.title ?? spine.find((t) => t.slug === slug)?.title)
    ...
    <div className="min-h-screen pt-20" style={{ background: "var(--bg)" }}>
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 pb-16">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div className="eyebrow">{mn ? "Хичээлүүд · 8-р анги" : "Courses · Grade 8"}</div>
        </div>

        <h1 className="serif" style={{ fontWeight: 400, fontSize: "clamp(32px, 5vw, 56px)", letterSpacing: "-0.04em", lineHeight: 1, color: "var(--fg)" }}>
          {mn ? "8-р ангийн сэдвүүд" : "Grade 8 Topics"}
        </h1>
        <p className="mt-3 mb-8" style={{ color: "var(--fg-1)", fontSize: 16, maxWidth: "56ch" }}>
          {mn
            ? "Алгебр жинхэнэ утгаараа эхэлдэг жил — бодит тоо, зэрэг, язгуур, шугаман тэгшитгэл ба функц, мөн график дээр амьдардаг өгөгдөл. Сэдэв бүрийг interactive, өөрийгөө шалгадаг хичээлүүдээр сууриас нь зааж өгнө."
            : "The year algebra begins in earnest — real numbers, exponents, roots, linear equations and functions, and the data that lives on a graph. Each topic is taught from the ground up with interactive, self-checking lessons."}
        </p>

        {/* Placement CTA / summary */}
        {placement ? (
          <Link href="/math/8/placement" className="card-edit p-4 mb-8 flex items-center gap-4" style={{ textDecoration: "none", borderColor: "var(--accent-line)", background: "var(--accent-wash)" }}>
        ...
        <ol className="space-y-3">
          {spine.map((t, i) => {
            const topic = t.live ? getGrade8TopicLocalized(t.slug, lang) : null;
