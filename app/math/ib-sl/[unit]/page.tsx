...
import { getIbSlUnit, getIbSlSpine } from "@/lib/genmath-data/ib-sl";

// An IB SL topic page: what it builds on, then the subtopic lessons in order.
export default function IbSlUnitPage() {
  const params = useParams();
  const unitSlug = params.unit as string;
  const unit = getIbSlUnit(unitSlug);
  const spineEntry = getIbSlSpine().find((u) => u.slug === unitSlug);
    ...
    return (
      <div className="min-h-screen pt-20 flex items-center justify-center" style={{ background: "var(--bg)" }}>
        <div className="text-center">
          ...
          <Link href="/math/ib-sl" className="btn btn-line mt-5 inline-flex items-center gap-1.5">
    ...
    <div className="min-h-screen pt-20" style={{ background: "var(--bg)" }}>
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 pb-16">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div className="eyebrow">IB Math SL (AA) · Topic {unit.unit}</div>
        </div>
        ...
        <ol className="space-y-3">
          {unit.lessons.map((lesson, i) => (
            <li key={lesson.slug}>
              <Link
                href={`/math/ib-sl/${unitSlug}/${lesson.slug}`}
          ...
          <>
            ...
            <div className="flex flex-wrap gap-3">
              {unit.practice.length > 0 && (
                <Link href={`/math/ib-sl/${unitSlug}/practice`} className="btn btn-primary">
                  Practice
                </Link>
              )}
              {unit.testYourself.length > 0 && (
                <Link href={`/math/ib-sl/${unitSlug}/test`} className="btn btn-line">
                  Test yourself
                </Link>
              )}
            </div>
