...
import { getCalcUnit, getCalcLesson } from "@/lib/genmath-data/calculus";
...
function CalcLessonPageInner() {
  ...
  const lesson = getCalcLesson(unitSlug, lessonSlug);
    ...
    return (
      <div className="min-h-screen pt-20 flex items-center justify-center" style={{ background: "var(--bg)" }}>
        <div className="text-center">
          ...
          <Link href={`/math/calculus/${unitSlug}`} className="btn btn-line mt-5 inline-flex items-center gap-1.5">
  ...
  return (
    <LessonPlayer
      ...
      baseHref={`/math/calculus/${unitSlug}`}
      crumb={`Calculus · Unit ${unit.unit} · ${unit.title}`}
    />
  );
}

// Content requires an account; the hub and unit pages above stay public.
export default function CalcLessonPage() {
  const params = useParams();
  const { lang } = useLang();
  const unitSlug = params.unit as string;
  return (
    <ContentGate courseKey="calculus" topicSlug={unitSlug} backHref={`/math/calculus/${unitSlug}`} backLabel={chrome("Back to unit", lang)}>
      <CalcLessonPageInner />
    </ContentGate>
  );
}
