...
// `lessonTotals` arrives as a plain prop from the server page — same pattern
// as the dashboard — so this client bundle never touches the course corpus.
function CourseProgress({ lessonTotals }: { lessonTotals: Record<string, number> }) {
  ...
  const lessonsTotal = lessonTotals[context] ?? null;
...
export default function CourseProgressView({
  ...
  return (
    <Suspense fallback={<div className="min-h-screen pt-20" style={{ background: "var(--bg)" }} />}>
      <CourseProgress lessonTotals={lessonTotals} />
    </Suspense>
  );
}
