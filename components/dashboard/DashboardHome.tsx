...
// Client half of /dashboard. The lesson-total map is computed by the
// SERVER page from the course registry, so this heavy-traffic page never
// ships the ~14 MB corpus just to draw its progress bars.
export default function DashboardHome({ lessonTotals }: { lessonTotals: Record<string, number> }) {
            ...
            <section className="mt-8">
              ...
              >
                {contextSummaries.map((s) => {
                  ...
                  const lessonsTotal = isCourse ? (lessonTotals[s.context] ?? null) : null;
