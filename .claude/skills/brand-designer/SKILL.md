...
- Mascot: **exists** — the "radiant figure": a circle head haloed by 12
  rays, floating above a capsule body. Direction chosen by the owner
  (hand sketch, 2026-08-03), superseding the animal candidates in the
  brief below; the brief's construction rules (geometric, crisp at 24px,
  silhouette test, flat color) still bind. Single source of geometry:
  `components/brand/mascot-geometry.mjs` — change THERE and regenerate;
  never hand-edit exports. In-app rig: `components/brand/Mascot.tsx`
  (`<Mascot pose/>`, `<HeadMark/>` logo mark, `<MascotSpinner/>` — rays
  orbit the head for loading, reduced-motion safe via globals.css).
  Wordmark: `components/brand/Wordmark.tsx` (head as MONGOL's O, figure
  as POTENTIAL's I). Static baked-color masters for off-site use:
  `public/images/mascot/*.svg`, regenerated by
  `node scripts/brand/export-mascot.mjs`. Poses: idle, wave, celebrate,
  think, point, sleep, walk. Name: NOT yet chosen by the owner — never
  ship a placeholder name into UI copy.
