...
- Mascot: **does not exist yet, but its direction is DECIDED** — the
  owner's hand sketch (2026-08-03): a "radiant figure" — a circle head
  haloed by rays, floating above a rounded body, no face. This
  supersedes the animal candidates in the brief below; the brief's
  construction rules (geometric, crisp at 24px, silhouette test, flat
  color) still bind. A first SVG implementation (commit 4c55bc6,
  reverted) was rejected by the owner as not matching the sketch closely
  enough — the next attempt must copy the sketch's proportions
  faithfully (get the owner's eyes on renders BEFORE wiring it into any
  page), and should keep that attempt's mechanics: one geometry source
  feeding the rig, the spinner (rays orbit for loading), the wordmark
  (head as MONGOL's O, figure as POTENTIAL's I), and static exports.
  Name: not chosen — never ship a placeholder name into UI copy.
