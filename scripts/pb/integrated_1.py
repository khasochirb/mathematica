...
from imbank import (P, closed, figure, fmt, form, lin, mk_num, mk_txt,  # noqa: E402
                    money, pt, seg)
import integrated_1_more as more  # noqa: E402
...
def build():
    ...
    # -----------------------------------------------------------------
    # Second half of every unit: twelve forms each, with a real ramp.
    # Six of the nine units stopped at Level 2 before this, so a student
    # who chose "Level 3 · Exam" was offered a tier that did not exist.
    # Generators in scripts/pb/integrated_1_more.py.
    # -----------------------------------------------------------------
    forms += [
        # --- Unit 1: quantities & the structure of expressions --------
        form("im1-unit-of-rate", "Naming the unit of a rate", 1, U1,
             "Dividing carries the units with it — read them off the division.",
             mk_txt("im1-uor", more.g_unit_of_rate())),
        form("im1-compound-convert", "Converting a rate twice over", 2, U1,
             "Two units change at once, so two conversion factors apply.",
             mk_num("im1-ccv", more.g_compound_convert())),
        form("im1-area-units", "Converting an area", 2, U1,
             "The length factor gets SQUARED, because two lengths were multiplied.",
             mk_num("im1-aun", more.g_area_units())),
        form("im1-rearrange-repeated", "When the unknown appears twice", 3, U1,
             "Gather the letter on one side and factor it out before dividing.",
             mk_txt("im1-rr2", more.g_rearrange_repeated())),
        form("im1-build-evaluate", "Build the expression, then use it", 3, U1,
             "A one-off charge is a constant; a per-unit charge is a coefficient.",
             mk_num("im1-bev", more.g_build_and_evaluate())),
        form("im1-interpret-parameter", "What a parameter means", 3, U1,
             "Each letter in a compound formula has a job — name it, do not guess.",
             mk_txt("im1-ipar", more.g_interpret_parameter())),

        # --- Unit 2: linear equations & inequalities -------------------
        form("im1-solve-negatives", "Solving with negatives", 1, U2,
             "Move the constant first, then divide — including by a negative.",
             mk_num("im1-sneg", more.g_solve_negatives())),
        form("im1-check-solution", "Checking a candidate solution", 1, U2,
             "Substitution always settles it, and it is never the wrong method.",
             mk_num("im1-chks", more.g_check_solution())),
        form("im1-clear-fractions", "Clearing the denominators", 2, U2,
             "Multiply EVERY term by the common denominator, not just the fractions.",
             mk_num("im1-clfr", more.g_clear_fractions())),
        form("im1-inequality-word", "A budget, in words", 2, U2,
             "A whole-object answer rounds DOWN, however close the next one is.",
             mk_num("im1-iwrd", more.g_inequality_word())),
        form("im1-abs-equation", "Absolute-value equations", 3, U2,
             "Two numbers sit a given distance from zero, so there are two cases.",
             mk_txt("im1-abse", more.g_abs_equation())),
        form("im1-compound-count", "Counting the integers between", 3, U2,
             "Solve all three parts at once, then remember strict bounds are excluded.",
             mk_num("im1-cpct", more.g_compound_count())),

        # --- Unit 3: functions & sequences -----------------------------
        form("im1-evaluate-f", "Evaluating a function", 1, U3,
             "f(x) is a NAME applied to an input, never f times x.",
             mk_num("im1-evf", more.g_evaluate_f())),
        form("im1-domain-range", "Domain and range from points", 2, U3,
             "Domain is the inputs, range the outputs — first and second coordinates.",
             mk_txt("im1-dmr", more.g_domain_range())),
        form("im1-is-function", "Is it a function?", 2, U3,
             "One output per input. Repeating an OUTPUT is allowed.",
             mk_txt("im1-isf", more.g_is_function())),
        form("im1-two-terms-arith", "A sequence from two terms", 3, U3,
             "Divide the term difference by the position difference to get d.",
             mk_num("im1-2ta", more.g_two_terms_arith())),
        form("im1-recursive-explicit", "Recursive against explicit", 3, U3,
             "Reaching the nth term takes n - 1 steps, not n.",
             mk_txt("im1-rex", more.g_recursive_explicit())),
        form("im1-sequence-threshold", "First term past a threshold", 3, U3,
             "'Exceeds' is strict — landing exactly on the level does not count.",
             mk_num("im1-sqth", more.g_sequence_threshold())),

        # --- Unit 4: linear functions ----------------------------------
        form("im1-slope-from-table", "Rate of change from a table", 1, U4,
             "Divide the jump in output by the jump in INPUT, which may not be 1.",
             mk_num("im1-sft", more.g_slope_from_table())),
        form("im1-line-from-intercepts", "A line from its two intercepts", 2, U4,
             "The y-intercept is the constant; the x-intercept is not.",
             mk_txt("im1-lfi", more.g_line_from_intercepts())),
        form("im1-compare-models", "When two plans cost the same", 2, U4,
             "Set the two expressions equal; the gap closes at the rate difference.",
             mk_num("im1-cmp2", more.g_compare_two_models())),
        form("im1-tiered-rate", "A tariff that changes rate", 3, U4,
             "Only the amount ABOVE the break is charged at the higher rate.",
             mk_num("im1-tier", more.g_tiered_rate())),
        form("im1-parallel-point", "Parallel through a point, then evaluated", 3, U4,
             "Same slope, new intercept — and the question asks for neither.",
             mk_num("im1-pll", more.g_parallel_through_point())),
        form("im1-model-breakdown", "Where the model stops making sense", 3, U4,
             "A model's domain is part of the model, not an afterthought.",
             mk_txt("im1-mbd", more.g_model_breakdown())),

        # --- Unit 5: systems -------------------------------------------
        form("im1-sys-substitution", "One substitution", 1, U5,
             "When an equation already gives y, the system is one line of algebra.",
             mk_num("im1-ssub", more.g_sys_substitution_easy())),
        form("im1-sys-check-pair", "Which pair solves BOTH", 1, U5,
             "A solution must satisfy every equation, not just the convenient one.",
             mk_txt("im1-scp", more.g_sys_check_pair())),
        form("im1-sys-mixture", "Two kinds, one total", 2, U5,
             "A count equation and a value equation, solved together.",
             mk_num("im1-smix", more.g_sys_mixture())),
        form("im1-sys-elimination-scaled", "Elimination that needs scaling", 2, U5,
             "When nothing cancels as written, multiply one equation first.",
             mk_num("im1-sels", more.g_sys_elimination_scaled())),
        form("im1-sys-combination", "When the question wants x + y", 3, U5,
             "Read what is asked before choosing a method — sometimes it is already there.",
             mk_num("im1-scmb", more.g_sys_combination())),
        form("im1-sys-inequality-point", "A point in BOTH regions", 3, U5,
             "Test every candidate in every inequality; passing one is not enough.",
             mk_txt("im1-sinq", more.g_sys_inequality_point())),

        # --- Unit 6: exponential functions -----------------------------
        form("im1-percent-to-factor", "Percentage to multiplier", 1, U6,
             "Add to 1 for growth, subtract from 1 for decay — never use the bare percent.",
             mk_num("im1-p2f", more.g_percent_to_factor())),
        form("im1-write-exp-model", "Writing the exponential model", 2, U6,
             "Starting amount out front, multiplier as the base, time as the exponent.",
             mk_txt("im1-wem", more.g_write_exp_model())),
        form("im1-compare-exp", "Two exponential models compared", 2, U6,
             "A bigger start does not settle it — the base wins eventually.",
             mk_num("im1-cxp", more.g_compare_exp_models())),
        form("im1-doubling-threshold", "First whole period past a level", 3, U6,
             "Whole periods mean the answer rounds UP.",
             mk_num("im1-dth", more.g_doubling_threshold())),
        form("im1-exp-two-points", "Recovering a and b", 3, U6,
             "t = 0 hands you a; the other point needs a root, not a division.",
             mk_txt("im1-x2p", more.g_exp_from_two_points())),
        form("im1-rate-period", "Same growth, different period", 3, U6,
             "The periods MULTIPLY to the annual factor — so take a root.",
             mk_txt("im1-rpd", more.g_rate_period_change())),

        # --- Unit 7: transformations, congruence & proof ---------------
        form("im1-reflect-axis", "Reflecting in an axis", 1, U7,
             "The axis you reflect in is the coordinate that SURVIVES.",
             mk_txt("im1-rfa", more.g_reflect_in_axis())),
        form("im1-rotate-origin", "Rotating about the origin", 1, U7,
             "A quarter turn swaps the coordinates and negates one of them.",
             mk_txt("im1-rot", more.g_rotate_about_origin())),
        form("im1-congruent-parts", "Corresponding parts", 2, U7,
             "A congruence statement is an ORDER — read the letters in it.",
             mk_txt("im1-cgp", more.g_congruent_parts())),
        form("im1-parallel-angle", "Angles on a transversal", 2, U7,
             "Alternate means equal; co-interior means supplementary.",
             mk_num("im1-pang", more.g_parallel_angle())),
        form("im1-angle-algebra", "Angle relationships as equations", 3, U7,
             "Corresponding angles are EQUAL — that is what makes an equation.",
             mk_num("im1-aalg", more.g_angle_algebra())),
        form("im1-triangle-theorems", "Triangle theorems in action", 3, U7,
             "Exterior angle, isosceles base angles, and the midsegment's halving.",
             mk_num("im1-tth", more.g_triangle_theorems())),

        # --- Unit 8: coordinate geometry -------------------------------
        form("im1-segment-length-axis", "Length along a grid line", 1, U8,
             "A horizontal or vertical segment needs one subtraction, not a formula.",
             mk_num("im1-sla", more.g_segment_length_axis())),
        form("im1-triangle-perimeter", "Perimeter from coordinates", 2, U8,
             "Three distances, added — and the hypotenuse is the one people drop.",
             mk_num("im1-tper", more.g_triangle_perimeter())),
        form("im1-other-endpoint", "The midpoint, run backwards", 2, U8,
             "Double the midpoint and subtract the endpoint you know.",
             mk_txt("im1-oend", more.g_other_endpoint())),
        form("im1-triangle-area-coords", "Area from three vertices", 3, U8,
             "The bracket gives TWICE the area — the half is not optional.",
             mk_num("im1-tac", more.g_triangle_area_coords())),
        form("im1-prove-parallelogram", "What proves a parallelogram", 3, U8,
             "Equal slopes on both pairs of opposite sides, and nothing more.",
             mk_txt("im1-ppg", more.g_prove_parallelogram())),
        form("im1-right-angle-condition", "Making the angle right", 3, U8,
             "Perpendicular means the slopes multiply to -1.",
             mk_txt("im1-rac", more.g_right_angle_condition())),

        # --- Unit 9: data & statistics ---------------------------------
        form("im1-mode-median", "Mode and median", 1, U9,
             "Sort before reading a median; the mode needs no sorting at all.",
             mk_num("im1-mmd", more.g_mode_median())),
        form("im1-compare-spread", "Same centre, different spread", 2, U9,
             "Equal means say nothing about how tightly the values cluster.",
             mk_txt("im1-csp", more.g_compare_spread())),
        form("im1-relative-frequency", "Percentages inside a table", 2, U9,
             "The words 'of the ...' name the denominator.",
             mk_num("im1-rfq", more.g_relative_frequency())),
        form("im1-residual-fit", "Residuals and their sign", 3, U9,
             "Observed minus predicted — reversing it reverses the story.",
             mk_num("im1-rsf", more.g_residual_fit())),
        form("im1-association-claim", "What an association licenses", 3, U9,
             "Correlation is real evidence of association and no evidence of cause.",
             mk_txt("im1-asc", more.g_association_claim())),
        form("im1-outlier-effect", "Adding one extreme value", 3, U9,
             "The mean feels a value's SIZE; the median only feels its position.",
             mk_txt("im1-oute", more.g_outlier_effect())),
    ]
