...
def emit(skills, by_id):
    ...
    lines = [
        "-- ЭЕШ skill graph — seed data.",
        "-- GENERATED by scripts/skills/esh_graph.py. Do not hand-edit; edit the builder",
        "-- and re-run, or the graph and the database drift apart silently.",
        "--",
        "-- name_mn is NULL on every row by design: Mongolian is authored in Phase 3 by a",
        "-- Mongolian-speaking maths teacher expressing the idea, never translated from the",
        "-- English here.",
        "--",
        f"-- {len(rows)} skills, {len(edges)} prerequisite edges.",
        "-- exam_weight is a percentage of the whole exam, derived from the measured share",
        "-- of each topic across 20 real past papers (data/esh/exam-weights.json).",
        "",
        "begin;",
        "",
    ...
    lines += [
        "on conflict (skill_id, prereq_skill_id) do update set strength = excluded.strength;",
        "",
        "commit;",
        "",
    ]
    os.makedirs(os.path.join(ROOT, "supabase", "seed"), exist_ok=True)
    with open(os.path.join(ROOT, "supabase", "seed", "esh_skills.sql"), "w") as f:
        f.write("\n".join(lines))
    return rows, edges
