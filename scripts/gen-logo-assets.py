...
def trace(mask_img, tmp):
    """potrace a PIL bilevel image; return (viewBox, transform, path d).

    POLARITY: potrace treats BLACK as the shape. Callers pass a mask that is
    True where the shape is; it is flipped to black-on-white here.
    """
    mask_img.point(lambda v: 0 if v > 127 else 255).convert("1").save(tmp)
    out = subprocess.run(
        ["potrace", tmp, "-s", "-o", "-", "--flat", "-a", "1.0", "-O", "0.2"],
        capture_output=True, text=True, check=True,
    ).stdout
    return (
        re.search(r'viewBox="([^"]+)"', out).group(1),
        re.search(r'<g transform="([^"]+)"', out).group(1),
        re.search(r'<path d="(.*?)"', out, re.S).group(1).strip(),
        out,
    )

# --- the mark vector ----------------------------------------------------
# Trace the alpha channel of the square canvas. The result is a filled
# outline, not a stroked path — visually identical, and fill="currentColor"
# is what makes it follow the theme.
view, transform, path_d, svg = trace(canvas.split()[3], "/tmp/mark-mask.pbm")
...
# --- the lockup vector: two traces in ONE coordinate system -------------
# The header shows the designer's whole lockup — mark AND wordmark — so
# both need tracing. A flat raster cannot do the job: the wordmark is
# black artwork and would disappear on the dark theme. Traced separately,
# the mark keeps var(--accent) and the wordmark takes var(--fg), the same
# token the old HTML wordmark used, so it flips with the theme exactly as
# the text did. Both masks are cut from the SAME padded crop, so potrace
# returns the same viewBox/transform for each and the two paths land in
# register; the assert below is the drift alarm for a changed export.
crop = rgb[y0:y1, x0:x1]
blue_mask = Image.fromarray(
    (((crop[:, :, 2] - crop[:, :, 0] > 60) & (crop[:, :, 2] > 120)) * 255).astype(np.uint8))
dark_mask = Image.fromarray(((crop.max(axis=2) < 128) * 255).astype(np.uint8))
lk_view, lk_tf, lk_mark_d, _ = trace(blue_mask, "/tmp/lockup-mark.pbm")
lk_view2, lk_tf2, lk_word_d, _ = trace(dark_mask, "/tmp/lockup-word.pbm")
assert (lk_view, lk_tf) == (lk_view2, lk_tf2), "lockup traces out of register"
lk_w, lk_h = (float(v) for v in lk_view.split()[2:])
...
# Header/footer/auth all render this. Two colour slots: the mark follows
# the accent, the wordmark follows --fg (near-black on light, near-white
# on dark) — the same token the HTML wordmark it replaces was using, so
# legibility on the dark theme is preserved, which a flat PNG of black
# lettering cannot do. aspect-ratio is baked in so `h-10 w-auto` sizes
# correctly without the caller knowing the lockup's proportions.
lockup_component = f'''// GENERATED by scripts/gen-logo-assets.py — do not edit by hand.
// The designer's full lockup — mark + "mongol potential academy" — traced
// from assets/brand/lockup-master.png as two paths in one coordinate
// system. Re-run the script after a new export.
//
// Two colours, both theme tokens: the mark follows the accent, the
// wordmark follows --fg (the token the old HTML wordmark used), so the
// lettering stays legible on the dark theme where the artwork's black
// would vanish. That is why this is a trace and not the PNG.

export default function LogoLockup({{
  className,
  markColor = "var(--accent)",
  textColor = "var(--fg)",
  title = "Mongol Potential Academy",
}}: {{
  className?: string;
  markColor?: string;
  textColor?: string;
  ...
  return (
    <svg
      ...
      aria-label={{title}}
      className={{className}}
      style={{{{ aspectRatio: "{lk_w:g} / {lk_h:g}" }}}}
      preserveAspectRatio="xMidYMid meet"
    >
      <g transform="{lk_tf}" stroke="none">
        <path style={{{{ fill: markColor }}}} d="{lk_mark_d}" />
        <path style={{{{ fill: textColor }}}} d="{lk_word_d}" />
      </g>
    </svg>
  );
}}
'''
Path("components/layout/LogoLockup.tsx").write_text(lockup_component)
...
print("components/layout/LogoMark.tsx, LogoLockup.tsx")
