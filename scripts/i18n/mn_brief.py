...
def teacher_brief(corpus, topic, glossary, theses=None):
    ...
    w("")
    w("Friendly and instructional — **«чи»**, the way you would speak to one student")
    w("in front of you, not **«та»**. The English has jokes and energy; keep that.")
    w("Do not flatten it into textbook prose.")
    w("")
    w("Quotes take «...». Proper names take their standard Mongolian forms. Product")
    w("names and symbols stay Latin.")
    w("")
    w("> **Open question, and it affects this brief.** The register is not settled")
    w("> across the site: grade 7 is written entirely in «чи», grade 8 almost")
    w("> entirely in «та», and the app's own screens are formal throughout. See")
    w("> `memory/mn-group1-audits.md` §1. Write in «чи» unless told otherwise.")
    w("")
    w("---")
    w("")
    w("## Writing maths")
