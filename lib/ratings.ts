...
export const COURSE_DEFAULT_ATTRIBUTE: Record<string, AttributeKey> = {
  ...
  "course:integrated-1": "algebra",
  "course:integrated-2": "algebra",
...
export const UNIT_ATTRIBUTE_OVERRIDES: Record<string, AttributeKey> = {
  ...
  // Integrated Math 1 (default algebra) — an integrated year spans four
  // attributes by design, so most of its units override the course default.
  "course:integrated-1/functions-and-sequences": "functions",
  "course:integrated-1/linear-functions": "functions",
  "course:integrated-1/exponential-functions": "functions",
  "course:integrated-1/transformations-and-congruence": "geometry",
  "course:integrated-1/coordinate-geometry": "geometry",
  "course:integrated-1/data-and-statistics": "probstats",
  "course:integrated-2/quadratic-functions": "functions",
  "course:integrated-2/similarity-and-dilations": "geometry",
  "course:integrated-2/right-triangle-trigonometry": "trigonometry",
  "course:integrated-2/circles": "geometry",
  "course:integrated-2/probability": "probstats",
...
export function allRatedUnits(): RatedUnit[] {
  ...
  const namedSpines: [string, { slug: string; title: string; live: boolean }[]][] = [
    ...
    ["course:integrated-1", IM1_SPINE],
    ["course:integrated-2", IM2_SPINE],
...
export function computeRatings(input: RatingsInput): RatingsProfile {
  ...
  const scoreAttrOneWay = (
    ...
    const scoreOf = (u: UnitRating) => (withBank ? u.score : u.scoreNoBank);

    // Displayed "worked N of M units" — bank practice counts as having
    // worked a unit, because it is work.
    const unitsTouched = attrUnits.filter((u) => (withBank ? u.touched : u.touchedNoBank)).length;
    ...
    if (streamUnits.length > 0)
      streams.push({ kind: "course", n: streamUnits.length, acc: courseMastery, perf: C.RATING_FLOOR + RANGE * courseMastery, conf: courseConf, weight: C.W_COURSE });
  ...
  const scoreAttr = (
    ...
    const withBank = scoreAttrOneWay(attrUnits, examAgg, placeAgg, true);
    const anyBank = attrUnits.some((u) => u.bankCredit > 0);
    if (!anyBank) return withBank;
    const noBank = scoreAttrOneWay(attrUnits, examAgg, placeAgg, false);
    return withBank.score >= noBank.score ? withBank : noBank;
  };
...
const COURSE_TITLES_EN: Record<string, string> = {
  ...
  "course:integrated-1": "Integrated Math 1",
  "course:integrated-2": "Integrated Math 2",
...
const COURSE_TITLES_MN: Record<string, string> = {
  ...
  "course:integrated-1": "Нэгдсэн математик 1",
  "course:integrated-2": "Нэгдсэн математик 2",
