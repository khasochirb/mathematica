...
# Phase 2b trigger: continuous_distribution split (2026-05-13)

Phase 3b.2 in-chat classification surfaced **4 CDF/PDF (continuous distribution) questions** currently force-fit to `discrete_distribution` at confidence 0.60:

- Test-2A-Q35 — continuous CDF, find P(3<X≤4)
- Test-2B-Q35 — continuous CDF variant
- Test-6A-Q31 — continuous PDF, find normalization k
- Test-6B-Q31 — continuous PDF, compute P(1<X<1.5)

These are one shy of the ≥5 systemic-gap threshold (per the 3b.2 surveillance rule). The taxonomy's `discrete_distribution` scope is "Probability tables, P(X=x) given a parametric distribution" — continuous PDFs/CDFs are conceptually distinct (integration over a density vs summation over a mass function), and a drill cohort on discrete tables doesn't teach the continuous skill.

**Trigger condition:** when Phase 2b textbook extraction (`ESH-nom.pdf`, paused per the Phase 2 status above) surfaces **≥1 more** continuous-distribution question, split out `continuous_distribution` as the 52nd skill tag and re-classify these 4 (plus any new ones).

**How to find them at re-tag time:** filter `scripts/skill-tag-classification.csv` for rows where `skill_tag=discrete_distribution AND confidence≤0.60 AND classifier=in-chat`. The confidence-0.60 force-fit floor is the marker.

**Proposed tag scope (for when triggered):**
