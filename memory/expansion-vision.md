...
9. **Hub consistency (Khas, repeatedly; enforced 2026-08-01).** The ЭЕШ, SAT and IB hub landing pages MUST share design and structure — the owner asked ~5 times before it was done, so this is not a preference, it's a standing requirement. All three build on `components/hub/HubKit.tsx` (HubShell/HubHero/HubSection/HubRowLink) in the canonical order **hero → tests → course → practice by topic → progress**, hub-specific extras only after. `lib/hub-consistency.test.ts` fails the build on drift. Any NEW hub (AP, AMC, …) starts from HubKit, no exceptions; content language stays a hub property (§4.7).
