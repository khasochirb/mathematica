...
5. **Pricing: FREE for the first few (re)launches** (Khas, 2026-06-16) — keep the whole product free through the early launches to grow audience + reputation; revisit a paid tier later once there's traction. So nothing is gated today; the loop and all content ship free. A pricing/model analysis ("go for the best option") is deferred until after the early launches.
   - **SUPERSEDED (Khas, 2026-08-01) — Premium prices are public.** "Let's put the prices already, we need premium accounts to be out there so that I won't be paying for API cost." Prices live in `lib/pricing.ts` (₮19,900/сар · ₮49,900/3 сар, single source of truth); the upgrade modal takes purchase requests into `premium_waitlist`; fulfillment is manual (Supabase dashboard or the ADMIN_ACTIVATION_KEY route — FLAG-003). AI tutor quota is tiered: 3/day free, 30/day Premium. The free past-paper core stays free.
...
7. **Language architecture.** ✅
   ...
   - **AMENDMENT (Khas, 2026-07-28) — ЭЕШ prep COURSES are English-first during the content buildout.** The taught courses at `/practice/esh/learn` ship in English so they can be curated immediately from the existing sympy-verified catalog; translation quality risk was the owner's stated reason for not leading with Mongolian. A Mongolian translation pass (scripts/i18n) comes later, over frozen content — each unit's `source` field in `lib/esh-course.ts` is that pass's work order. Everything else in the ЭЕШ hub (past papers, drills, chrome, analytics) remains Mongolian per the original rule. Companion decisions: the prep courses assume ~650/800 (`ESH_COURSE_READY_SCORE`); students projecting below that are routed to General Math by `components/esh/course/EshLevelAdvisor.tsx`.
...
9. **Hub consistency — the ЭЕШ design is the REFERENCE (Khas, repeatedly; direction corrected 2026-08-01).** The ЭЕШ, SAT and IB hub landing pages MUST share design and structure, and the direction is fixed: **SAT and IB are built to look like the original ЭЕШ hub** (wide `max-w-5xl` shell, bordered stats header, accent progress banner, 2×2 icon action-card grid), never the reverse. A first attempt flattened ЭЕШ into a simpler row-link style; the owner explicitly rejected it ("i asked you to make sat and ib hubs like the old eysh design"). All three build on `components/hub/HubKit.tsx` (HubShell/HubHeader/HubProgressBanner/HubActionGrid — primitives extracted FROM the ЭЕШ page); `lib/hub-consistency.test.ts` fails the build on drift. Any NEW hub (AP, AMC, …) starts from HubKit; content language stays a hub property (§4.7). Companion decisions (same date): ЭЕШ topic pages carry NO legacy formula sheets or tips — course first, problem bank directly below; the AI tutor lives ONLY on the post-exam missed-problem flow (RefinementLoop) where it explains the miss and routes to practice — never inside course lessons; the ЭЕШ label is «Шалгалт», not «Бодит шалгалт».
