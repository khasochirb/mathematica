...
5. **IM3 completion** — units 3–8, bank, exams (pre-existing roadmap).
   ◐ In progress: the course is LIVE at /math/integrated-3 with units
   1–4 open. Unit 3 "Exponential & Logarithmic Functions" and unit 4
   "Trigonometric Functions" authored 2026-07-31
   (scripts/im/build_im3_unit3.py, build_im3_unit4.py — 308 sympy
   checks between them); units 1–2 were authored earlier but never
   wired. Remaining: units 5–8 in spine order
   (function-families-and-inverses next), then the IM3 bank
   (scripts/pb) and 3 exams via the IM selector once every unit is
   live.
