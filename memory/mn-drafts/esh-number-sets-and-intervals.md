...
## The bracket finding — lesson 2's whole reason for being rewritten

The English lesson teaches one interval convention: $(a, b)$ open,
$[a, b]$ closed. **The ЭШ papers use two.** Counting only the bracket shapes
that actually distinguish the conventions — closed $[a,b]$ is identical in both,
so it tells us nothing, and round-round is unusable as evidence because
coordinate pairs share its shape:
...
**105 reversed against 46 standard.** Real examples from the papers:
`$]-\infty;\ 3[$`, `$]4;\ +\infty[$`, `$[10;12[$`, `$]0, 2]$` — alongside
`$[3; \infty)$` and `$(0, 1/2]$`. Note the semicolon separator too, which the
English never uses.

Mongolia's school mathematics inherited the Russian convention, so this is not
a stray typesetting habit — it is what a student sees. The ministry standard
does not settle it: А/492 uses «завсар» but fixes no bracket notation.

**What I did with that.** Unlike the complement question, this one does not have
a single right answer to switch to — the exam genuinely uses both, so a student
must be able to *read* both. Teaching one and ignoring the other is the actual
defect in the English lesson, and it would still be a defect if I flipped which
one was taught. So lesson 2 teaches the equivalence explicitly
($]a, b[$ is $(a, b)$), keeps $(a, b)$ as the form the lessons write in — the
rest of the site, the SAT hub and the IB hub all use it, and IB is right to —
and adds the semicolon separator as something to expect.

If you would rather the ЭШ hub write in the reversed convention throughout, that
is a bigger change than the complement switch was: it touches this topic, every
inequality topic, and the calculus units. Worth your explicit call, which is why
it is here and not buried in the notes.
...
**COMPARISON:** Автобусны маршрут дахь буудлууд: дөрвөлжин хаалт бол автобус
буудал ДЭЭР зогсоно, дугуй хаалт бол зогсолгүй өнгөрнө гэсэн үг. Хооронд нь
байгаа зам хоёр тохиолдолд ч үйлчилнэ.
...
**KEY IDEA:** Дөрвөлжин хаалт төгсгөлийг оруулна, дугуй хаалт оруулахгүй;
$\infty$ үргэлж дугуй. Урт нь $b - a$, хаалтнаас хамаарахгүй — харин доторх
бүхэл тооны ТОО хамаарна.

**TEACHING:**

$[a, b]$ хоёр төгсгөлийг хоёуланг нь оруулна ($a \le x \le b$); $(a, b)$
хоёуланг нь хасна ($a < x < b$); $[a, b)$ ба $(a, b]$ холимог. Дөрвөлжин хаалт
төгсгөл дотор, дугуй хаалт гадна. Тооны шулуун дээр дүүрэн цэг ба хоосон цэг.

**Хоёр өөр бичиглэлийг хоёуланг нь таних хэрэгтэй.** ЭШ-ийн хувилбарууд
төгсгөлийг хасахдаа дугуй хаалт биш, **эргүүлсэн дөрвөлжин хаалт** ихэвчлэн
хэрэглэдэг:
...
$(3, \infty)$, $x \le -1$ нь $(-\infty, -1]$. $\infty$ нь ҮРГЭЛЖ дугуй (эсвэл
эргүүлсэн) хаалттай — энэ нь чиглэл болохоос хүрч болох төгсгөл биш. Бүхэл
...
2. **Эргүүлсэн хаалтны бичиглэл** —
   ...
   ЭШ-ийн хувилбарууд үүнийг байнга хэрэглэдэг: хаалт гадагш харвал төгсгөл
   орохгүй.
