...
> depends on it (6aw).
>
> **Next queue, non-ЭШ Mongolian mirrors:** grade 9 (3 left), 10 (4), 11 (4),
...
seventy-six drafts carry zero em-dash parentheticals in shipping prose, and
...
| 2 | **2d** | Does the decimal comma apply inside `$...$`? | none: your call | **1,631** math-mode decimals, one scripted pass + a render walk |
...
### 2d. Math-mode decimals — **not applied. Surveyed: 1,631 of them.**
...
### 6ar. The derivative unit teaches four codes mapped elsewhere, and one English fact is false

**Found 23 Sep 2026, drafting `calculus/the-derivative` (draft 63), ЭШ
Анализын эхлэл unit 2.**

- **Mapping** (`lib/esh-course.ts:259`: 11.9б, 11.9в, 11.9г, 11.9е). All four
  taught, 11.9е by half: the tangent-line equation once (`cal2-ty-4`), normals
...
- **A false fact in the English**: lesson 2's fact "f ↗ ⟺ f′ > 0". The
  backward direction fails for x³ (increasing, f′(0) = 0), which the same
  lesson's `cal22-t2` teaches. Fix: "f′ > 0 ⇒ f ↗". The draft carries the
  latex unchanged.
- **A miscount**: lesson 4 step 6 is titled "All four in one expression", but
  `cal24-we2` uses three rules and no trig. The Mongolian says three.
...
### 6au. Integrals: the exam's commonest integral has no example, and three mapped codes are untaught

**Found 23 Sep 2026, drafting `calculus/integrals` (draft 66), ЭШ Анализын
эхлэл unit 5.**

- **16 of the bank's 36 integral questions have a linear inner function**:
  $\int 12(4x+1)^2\,dx$ (2023), $\int \frac{dx}{\sqrt{3x+1}}$ (2024), $\int
  \cos 2x\,dx$ (2025), and $\int (1 - 2e^{-x})\,dx$ (2022), each in all four
  papers of its year. Substitution handles them, but the course has no example
  with u = ax + b (11.10в, 12.8а). One worked example in lesson 4 would cover
  the largest block of ЭШ integrals. A further 4 (2024,
  $\int \frac{e^{2x}}{e^x \pm 1}\,dx$) need substitution followed by division.
- **Untaught but mapped** (`lib/esh-course.ts:264`): 12.8б (partial fractions:
  `test4a`, `test4b` need it), 12.8д (elective: trig integrals), 12.8е
  (elective: by parts). As in 6as, the claim hides them from the
  untaught-codes list.
- **Three English errors**: Archimedes did not name the "method of
  exhaustion" (17th century), and his result is a parabolic *segment*; the FTC
  funFact's "routinely voted near the top" and "eleven symbols" cannot be
  sourced; `cal53-we2` calls cosine on [0, π/2] "the first hump" (half of it).
- **Term choice to confirm**: «эх функц» (dictionary) over the bank's
  «антиуламжлал»; «Анализын үндсэн теорем» with «Ньютон-Лейбницийн томьёо»
  beside it.

### 6av. Calculus closed: nine mapped ministry codes are taught nowhere, and the whole spine needs one re-map

**Found 23 Sep 2026, drafting `calculus/applications-of-integrals` (draft
67), the last unit of ЭШ Анализын эхлэл.**

- **This unit**: 11.10и (volumes of revolution), 11.10к (improper integrals)
  and 12.8ж (trapezoidal rule), all electives, are mapped here
  (`lib/esh-course.ts:268`) and taught nowhere (0 hits). 11.10з is taught in
  unit 5 (Riemann sums), not here.
- **Across the six calculus units**, nine codes are claimed and never taught:
  12.7г (6as); 11.9и, 11.9м (6at); 12.8б, д, е (6au); 11.10и, к, 12.8ж
  (above). Two more are taught only in part: tg x of 12.7а (6ar), and the
  linear inner function of 11.10в / 12.8а (6au). A unit claims each one, so
  none of them appear in the untaught-codes list that `lib/esh-course.test.ts`
  asserts. The taught codes also sit one unit behind where they are taught
  (6aq–6at). One ship-mode pass should re-map units 1–6 together.
- **Examined but untaught**: implicit differentiation (4 bank questions, 6as)
  and partial fractions (2, 6au).
- **English fixes carried in the Mongolian**: the phone funFact claimed step
  counters and screen rotation integrate acceleration (they do not; inertial
  navigation does, which the Mongolian says); the capstone's "the full calculus
  core of the ЭШ" becomes «гол цөм»; `cal64-we2` listed an endpoint as a
  turnaround.
- **Term to confirm**: «үйлчлэх утга» for RMS.

### 6aw. The ЭШ spine runs Вектор ба матриц units 1 and 2 in the wrong order

**Found 23 Sep 2026, drafting `vectors-matrices/vectors-and-coordinates`
(draft 68).**

- `lib/esh-course.ts` lists `vector-arithmetic` as ЭШ unit 1 and
  `vectors-and-coordinates` as unit 2, following the ministry's code order
  (10.9а, adding and scaling, before 10.9в, coordinates and length). The home
  course (`VECMAT_SPINE` in `lib/genmath-spines.ts`) has them the other way
  round, and `vector-arithmetic` depends on the coordinates unit: its
  `buildsOn` is "Components and magnitude from Unit 1", lesson 2 refers back
  twice to "Unit 1", and it computes everything in components. **Ship-mode
  fix: swap `live(1, …)` and `live(2, …)`**; the code mapping can stay.
- Because a Mongolian mirror renders in both routes, where "Unit 1" means
  different units, the drafts name the unit («Вектор ба координат» нэгж)
  instead of numbering it.
- **Term to confirm**: «коллинеар» (ministry 10.9а, 10.9е) for parallel
  vectors, over the bank's «параллель» (2), with «нэг шулуун дээр орших
...
- **Gate gap** (found drafting draft 70): `mn_draft_check.py` catches a `$…$`
  span broken across lines but not a `$$…$$` display span. Four had slipped
  into drafts 69–70 through a rewrap; all are fixed. The check should cover both
  delimiters (ship-mode, with 6ap's mixed-script check).

### 6ay. Vectors in space: 12.5г is examined, half-taught and listed as untaught; three typos in the live bank

**Found 23 Sep 2026, drafting `vectors-matrices/vectors-in-space` (draft 71).**

- **12.5г** (the plane ax + by + cz = d, "understand and write") is on the
  untaught-codes list in `lib/esh-course.ts`, but lesson 4 of this unit
  teaches the *understand* half: reading the normal off the equation, and
  parallel and perpendicular planes. The *write* half is missing and is
  examined: two bank questions ask for a plane's equation (through three
  points; through a point with a given normal). Either claim the code and add a
  lesson on writing the equation, or keep it listed with a truer reason.
- **11.8д** (elective: direction angles with the axes) is mapped here and not
  taught.
- **Three mixed-script words in live `data/questions/`**, found by scanning every
  bank file: `зaa` (`test6a`, Latin a's), `Ногоony` (`test1b`, Latin y),
  `оршиx` (`test2b`, Latin x). Same class as 6ad; the 6ap check should run over
  the bank too.
...
**Found 23 Sep 2026, drafting `vectors-matrices/transformation-matrices` (draft 74).**

- **Mapping clean.** 10.11а–ж, all seven taught.
- **Composition.** The ministry says «дараалсан хувиргалт» (10.11е); the
  dictionary, `GEOMETRY-TERMS.md` §1, draft 73 and this draft say «угсраа
  хувиргалт». I read the ministry's phrase as a description, not a term, and
  kept the settled word. Say if the ministry should win.
- **Reflection is «шулууны хувьд тэгш хэм»**, beside «цэгийн тэгш хэм»: the
  ministry's own pair (10.11б), and the bank's form. The vectors-and-matrices
  strand now agrees with itself and differs from `geometry-transformations`
  («тусгал») until 4g is ruled.
- **Coined: «налуулалт»** (shear), used twice as a named example, never
  examined. **Calqued: «нэгэн төрлийн координат»** (homogeneous coordinates,
  from the Russian), which carries lesson 3.
...
### 6be. Grade 9 inequalities: a wrong course pointer, and three names for one circle

**Found 23 Sep 2026, drafting `9/inequalities-and-absolute-value` (draft 76), the first non-ЭШ draft after the ЭШ course closed.**

- **English, ship mode:** lesson 6 teach step 1 says optimization comes in
  "Grade 12+". It is lesson 6 of `9/inequalities-in-two-variables`, the next
  grade 9 topic. The Mongolian points there.
- **Open and closed markers now have three names.** On a number line the
  shipped grade 7 mirror says «задгай / битүү тойрог», and this draft follows
  it (2g). `9/piecewise-and-absolute-value-graphs` says «задгай / битүү цэг» for
  graph endpoints, and `algebra-1/inequalities` says «будсан / будаагүй цэг»
  (6af). One line settles whether they should be one.
- **Rebuilt mnemonic:** the English "less-th**AN**d / great**OR**" is wordplay;
  the Mongolian uses «**БАГ**а бол **БАГ**тана» and «**ИХ** бол **ИХ**эр
  сүүл». Same class as 5d.
- **Coined:** «ашиг алдагдалгүй цэг» (break-even point).
